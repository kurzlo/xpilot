...
local battAlertLevel = { [1] = 30, [2] = 20, [3] = 10, [4] = 0 }
...
local function alertNotify(tag, xpilot)
  local cfg = xpilot.cfg
  local alert = cfg and cfg.get("alert")
  return (not alert.mute or alert.mute < 2) and (not alert[tag] or alert[tag] > 1)
end

local function alertBatt(alertData, xpilot)
  if alertNotify("batt", xpilot) then
...
local function alertCurr(alertData, xpilot)
  if alertNotify("curr", xpilot) then
...
local function alertGPS(alertData, xpilot)
  if alertNotify("gps", xpilot) then
...
local function alertFM(alertData, xpilot)
  local notifyFM = alertNotify("fm", xpilot)
  local notifyTelem = alertNotify("telem", xpilot)
  if notifyFM or notifyTelem then
